...
function calendar(YY, change_month, semi){    
    var DDD = new Date(YY, change_month, 0);
    var ldate = DDD.getDate();
    var lday = DDD.getDay();
    var fday = (7+lday - (ldate-1)%7)%7;
    var semi_f_day = (7+lday - (ldate-15)%7)%7;
    var semi_l_day = (7+lday - (ldate-16)%7)%7;
    var step1;
    var step2;
    if(semi==0){
        $('.number___dYpbUZlm.f_year').html(YY);
        $('.number___dYpbUZlm.f_month').html(change_month);
        $('.number___dYpbUZlm.f_date').html("1");
        $('.f_day_letter').html(WeekList[fday]);
        $('.f_day_letter').attr('style', 'color:'+colorList[fday]);
        $('.number___dYpbUZlm.l_year').html(YY);
        $('.number___dYpbUZlm.l_month').html(change_month);
        $('.number___dYpbUZlm.l_date').html("15");
        $('.l_day_letter').html(WeekList[semi_f_day]);
        $('.l_day_letter').attr('style','color:'+ colorList[semi_f_day]);
        step1 = 1;
        step2 = 15;
    }else if(semi == 1)
    {
        $('.number___dYpbUZlm.f_year').html(YY);
        $('.number___dYpbUZlm.f_month').html(change_month);
        $('.number___dYpbUZlm.f_date').html("16");
        $('.f_day_letter').html(WeekList[semi_l_day]);
        $('.f_day_letter').attr('style', 'color:'+colorList[semi_l_day]);
        $('.number___dYpbUZlm.l_year').html(YY);
        $('.number___dYpbUZlm.l_month').html(change_month);
        $('.number___dYpbUZlm.l_date').html(ldate);
        $('.l_day_letter').html(WeekList[lday]);
        $('.l_day_letter').attr('style', 'color:'+colorList[lday]);
        step1 = 16;
        step2 = ldate;
    }
